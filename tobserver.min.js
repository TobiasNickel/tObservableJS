...
c);else this.setRoot(a,c)};a.prototype.setRoot=function(a,c){var g=b(a);g.path||(g.path=[""]);for(var e in g.path)-1==g.path[e].indexOf(c)&&(g.path[e]=c+"."+g.path[e])};a.prototype.updateRootPath=function(a,b,g){if(a&&b&&g){a=a.children;for(var e in a)if(a[e].attr)for(var h in a[e].attr.path){var k=a[e].attr.path[h].replace(g+".","");"htmllist"==a[e].attr.type.toLowerCase()&&this.updateRootPath(a[e],k+"."+k,g+"."+k);f.off(a[e].attr.path[h]+"."+a[e]._tName);f.on(b+"."+k,a[e]._tObserver);a[e].attr.path[h]=
