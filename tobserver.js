        ...
        with(styleKeys) {
            try {
                ...
                try {
                    ...
                    attr = {
                        path: attr
                    }; // hand over a string, containing a path
                }
            }
        }
...
var tobserver = (function (window, document, undefined) {
    ...
    Object.defineProperty(Array.prototype, "indexOfIn", {
        enumerable: false
    });
            ...
            for (var i in commands) {
                ...
                if (this.socket && commands[i].online && !this.isLocal(commands[i].path)) {
                    ...
                    if (!commands[i].socket) {
                        this.socket.emit("tOmand", JSON.stringify({
                            path: commands[i].path,
                            data: commands[i].data,
                            run: commands[i].run
                        }));
                    } else {
                        commands[i].socket.broadcast.emit('tOmand', JSON.stringify({
                            path: commands[i].path,
                            data: commands[i].data,
                            run: commands[i].run
                        }));
